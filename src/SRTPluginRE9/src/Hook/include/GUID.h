...
#ifndef SRTPLUGINRE9_GUID_H
#define SRTPLUGINRE9_GUID_H
...
namespace SRTPluginRE9::Guid
{
	inline const std::string ToString(const GUID &guid) noexcept
	{
	...
	inline constexpr char ParseHexChar(const char &c)
	{
	...
	template <class T>
	inline constexpr T ParseHexType(const char *ptr)
	{
	...
	inline constexpr GUID ToGUID(const char *begin)
	{
		GUID result{};
	...
	template <size_t N>
	inline constexpr GUID ToGUID(const char (&str)[N])
	{
	...
	{
		inline constexpr GUID operator""_guid(const char *str, size_t N)
		{
			using namespace std::string_literals;
